...
function changeLocation(data) {
  ...
  console.log(
    "entryptedRegion",
    `${region}|${new Date(Date.now())}`,
    entryptedRegion
  );
  callAPI(
    `https://api.getsetgo.fitness/base_ind/API/v1/user_packages/${entryptedRegion}`,
    "get",
    (data) => {
      console.log(data);
    },
...
function encryption(dataToBeEncrypted) {
  ...
  encrypted_string = aes
    .encrypt(dataToBeEncrypted, secret_key, { iv: secret_iv })
    .toString()
    .replace(/\//g, "_")
    .replace(/\+/g, "-");
